...
def analyze_text(sentences: list[str]) -> dict:
    """
    ...
    if equation:
        ...
        try:
            if "exp(" in equation:
                ...
                import math
                # Extract parameters
                match = re.search(r'([\d.]+)\s*\*\s*exp\(([-\d./x]+)\)', equation)
                if match:
                ...
                if match:
                    ...
                    return constant / max(input_value, 0.1)  # Avoid division by zero
            else:
                # Linear: y = mx + b
                match = re.search(r'y\s*=\s*([-\d.]+)\s*\*\s*x\s*([+-]\s*[\d.]+)?', equation)
                if match:
                ...
                match = re.search(r'y\s*=\s*([\d.]+)\s*([+-])\s*([\d.]+)\s*\*\s*x', equation)
                if match:
