...
export function SimulationPanel({
            ...
            <div className="h-full flex flex-col p-6">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-400">
                    ...
                    <p className="text-sm max-w-[240px]">
                        Click on a connection line (edge) in the graph to simulate the causal relationship.
                    </p>
                </div>
            </div>
        );
    ...
    return <ActiveSimulation state={{ edge: selectedEdge, source: sourceNode, target: targetNode }} onClose={onClose} />
}

function ActiveSimulation({ state, onClose }: { state: { edge: RelationshipEdge, source?: ConceptNode | null, target?: ConceptNode | null }, onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        let strength = 1.0;
        if (edge.description.toLowerCase().includes("strong") || edge.description.toLowerCase().includes("key")) strength = 1.5;
        if (edge.description.toLowerCase().includes("weak") || edge.description.toLowerCase().includes("minor")) strength = 0.5;
            ...
            <div className="flex-1 flex flex-col gap-6">
                ...
                <div className="bg-slate-800/40 rounded-xl p-5 border border-blue-500/20 shadow-[0_4px_20px_rgba(59,130,246,0.05)] transition-all hover:border-blue-500/40">
                    <div className="flex justify-between text-sm text-slate-400 mb-2">
                        ...
                        <span className="text-blue-400 font-mono font-bold">{inputValue}%</span>
                    </div>
                    <div className="text-xl font-bold text-white mb-1 truncate" title={source?.label}>
                ...
                <div className="flex flex-col items-center justify-center gap-1 text-slate-500 py-2 relative">
                    ...
                    <div
                        className={`absolute top-0 bottom-0 w-[2px] bg-gradient-to-b from-blue-500 to-purple-500 -z-10 transition-opacity duration-300 ${isAnimating ? 'opacity-100' : 'opacity-30'}`}
                    ></div>

                    <div className="bg-[#050510] border border-white/10 px-3 py-1.5 rounded-full flex items-center gap-2 shadow-xl z-10">
                        {edge.relationship_type === 'direct' ? <TrendingUp className="w-4 h-4 text-emerald-400" /> : <TrendingDown className="w-4 h-4 text-rose-400" />}
                ...
                <div className="bg-slate-800/40 rounded-xl p-5 border border-purple-500/20 shadow-[0_4px_20px_rgba(168,85,247,0.05)] transition-all hover:border-purple-500/40">
                    <div className="flex justify-between text-sm text-slate-400 mb-2">
                        ...
                        <span className="text-purple-400 font-mono font-bold transition-all duration-300" style={{ transform: isAnimating ? 'scale(1.1)' : 'scale(1)' }}>
                            {outputValue.toFixed(1)}%
                        </span>
                    </div>
                    <div className="text-xl font-bold text-white mb-1 truncate" title={target?.label}>
