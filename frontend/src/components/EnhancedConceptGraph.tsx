...
export function EnhancedConceptGraph({
  ...
  useEffect(() => {
    ...
    if (svg.select("g.main-group").empty()) {
      ...
      ["pink", "blue", "white"].forEach((color, i) => {
        const colors = [NODE_COLORS.primary, NODE_COLORS.secondary, NODE_COLORS.tertiary];
        defs.append("marker")
          .attr("id", `arrow-${color}`)
  ...
  useEffect(() => {
    ...
    let nodes: D3Node[] = graphData.concepts.map((c, i) => {
      ...
      const existing = simulationRef.current?.nodes().find(n => n.id === c.id);
      return existing ? Object.assign(existing, c) : { ...c, colorIndex: i % 3 };
    });
    ...
    const linkEnter = link.enter().append("line")
      .attr("stroke", d => d.relationship_type === "direct" ? "#60a5fa" : "#fb923c")
      .attr("stroke-width", 1.5)
      .attr("stroke-opacity", 0)
      .attr("stroke-dasharray", d => d.relationship_type === "inverse" ? "5,5" : "none")
      .attr("marker-end", d => d.relationship_type === "direct" ? "url(#arrow-blue)" : "url(#arrow-pink)");
    ...
    nodeMerge.select("circle.main")
      .attr("stroke-width", d => d.id === selectedNodeId ? 3 : 2)
      .on("mouseenter", function (_event, _d) {
        d3.select(this).transition().duration(200).attr("r", 32).attr("stroke-width", 3);
      })
      .on("mouseleave", function (_event, d) {
        d3.select(this).transition().duration(200).attr("r", 28).attr("stroke-width", d.id === selectedNodeId ? 3 : 2);
      });
  ...
  }, [graphData, dimensions, selectedNodeId, handleNodeClick]);
