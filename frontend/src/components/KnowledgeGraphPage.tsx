...
import { Loader2, MessageSquare, MousePointer2 } from "lucide-react";
import type { GraphData, D3Node, ConceptNode } from "@/types";
...
export function KnowledgeGraphPage({
  ...
  const getVisibleGraphData = useCallback(() => {
    ...
    const filteredConcepts = graphData.concepts.filter(c => visibleNodeIds.has(c.id));
  ...
  useEffect(() => {
    if (initialDocumentId && initialDocumentTitle && initialDocumentId !== documentId) {
  ...
  }, [initialDocumentId, initialDocumentTitle, loadGraph, documentId, graphData, isLoadingGraph]);
  ...
  const handleNodeExpand = useCallback((node: D3Node) => {
    if (!graphData) return;
  ...
  return (
    <div className="flex h-full w-full bg-[#050510] text-white overflow-hidden">
      ...
      {/* Right Panel - Chat */}
      <div className="w-[400px] flex-shrink-0 h-full bg-[#0a0a1a] border-l border-white/10 flex flex-col">
        {/* Chat Header */}
        <div className="p-4 border-b border-white/10 bg-gradient-to-r from-pink-500/10 to-blue-500/10">
          <div className="flex items-center gap-2">
            <MessageSquare className="w-5 h-5 text-pink-400" />
            <h2 className="font-semibold text-white">Knowledge Chat</h2>
          </div>
          {selectedNode && (
            <p className="text-sm text-white/60 mt-1">
              Context: <span className="text-pink-400">{selectedNode.label}</span>
            </p>
          )}
        </div>
          ...
          {selectedNode && documentId ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-6">
              <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center mb-4">
                <MousePointer2 className="w-8 h-8 text-white/30" />
              </div>
              <h3 className="text-lg font-medium text-white/80 mb-2">
                Select a Node
              </h3>
              <p className="text-sm text-white/40 max-w-[280px]">
                Click on any node in the knowledge graph to start asking questions
                about that concept.
              </p>
            </div>
          )}
